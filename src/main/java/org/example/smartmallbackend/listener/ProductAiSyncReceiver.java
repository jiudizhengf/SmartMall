...
public class ProductAiSyncReceiver {
    ...
    /**
     * 监听商品 AI 同步队列
     * 消息格式 JSON: {"id": 101, "type": "UP"} 或 {"id": 101, "type": "DOWN"}
     */
    @RabbitListener(queues = RabbitMqConfig.PRODUCT_AI_SYNC_QUEUE)
    public void handle(String msgStr, Message message, Channel channel) throws IOException {
        log.info("收到原始消息内容: {}", msgStr);
        long deliveryTag = message.getMessageProperties().getDeliveryTag();
        log.info("收到商品AI同步消息: {}, deliveryTag: {}", msgStr, deliveryTag);
        try {
            JSONObject json= JSONUtil.parseObj(msgStr);
            Long spuId = json.getLong("spuId");
            ...
            log.error("处理商品AI同步消息失败: {}", e.getMessage());
            // 发生异常，拒绝消息并丢弃 (false, false)，或者重回队列 (false, true)
            // 这里为了防止死循环，选择丢弃
            channel.basicReject(deliveryTag,false);
        }
    }
    public void handleUp(Long spuId){
    ...
    public void handleDown(Long spuId){
